...
class Kinematics(object):
    ...
    def __init__(self):
        ...
        rospy.init_node("pyrobot_kinematics_vx300s")

        self.ik_srv = rospy.Service("pyrobot/vx300s/ik", IkCommand, self.ik_server)
        self.fk_srv = rospy.Service("pyrobot/vx300s/fk", FkCommand, self.fk_server)
        rospy.spin()

    def _init_kinematics(self):
        # Ros-Params
        base_link = rospy.get_param("pyrobot/vx300s/base_link")
        gripper_link = rospy.get_param("pyrobot/vx300s/gripper_link")
        robot_description = rospy.get_param("pyrobot/vx300s/robot_description")
        ...
        self.arm_joint_names = [u'waist', u'shoulder', u'elbow', u'forearm_roll', u'wrist_angle', u'wrist_rotate']
        self.arm_link_names = [u'vx300s/shoulder_link', u'vx300s/upper_arm_link', u'vx300s/upper_forearm_link', u'vx300s/lower_forearm_link', u'vx300s/wrist_link', u'vx300s/gripper_link', u'vx300s/ee_arm_link']
        self.arm_dof = 6
